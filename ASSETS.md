...
| badge-giocatore-attivo.png | badge | fatto (aggiornato) | cornice ovale dorata singola. Ricevuta @1024×1024 il 10/08: il file dichiarava trasparenza ma la scacchiera era in realtà "bruciata" nei pixel (canale alpha sempre 255) — ricostruita la trasparenza vera per saturazione colore (pixel desaturati/grigi → trasparenti, pixel dorati saturi → opachi), poi ritagliata sulla cornice e ridotta a 96×94 in `public/assets/`. Integrazione in `style.css` invariata (icona 13px prima del nome del giocatore attivo) |
